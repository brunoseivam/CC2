...
int sem_pending_insert(char* string, sem_category category)
{
   ...
   /* Ponteiro para entrada. Será usado em uma checagem adiante */
...
/* Atualiza uma informação para as entradas pendentes. */
void sem_pending_update(sem_pending_upd_type upd, void* value)
{
   ...
   switch(upd)
   {
      ...
      */
		...
		/* Atualiza o ponteiro para mais informações das entradas pendentes */
      case sem_upd_more_info:
         sem_gl_info.current_table->pending_changes->more_info = value;
         break;

		/* Insere um parâmetro na lista de parâmetros da função/procedimento pendente.
		   O parâmetro inserido ainda será carente de tipo */
      case sem_upd_param_insert:
		...
		/* Atualia o tipo dos parâmetros sem tipo. Os parâmetros sem tipo estarão na cauda da lista.
			Assim, basta percorrer a lista de trás pra frente, atualizando os tipos até encontrar o
			 primeiro parâmetro que já contenha tipo. */
...
/* Aplica de fato as alterações pendentes nas entradas carentes de tipo, que estão na pilha
    pending_stack */
void sem_pending_commit(void)
{
   ...
   while((entry = stack_pop(sem_gl_info.current_table->pending_stack)))
   {
      ...
      entry->more_info     = sem_gl_info.current_table->pending_changes->more_info;
		...
		/* Lembrando que não precisa inserir esta entrada na árvore, pois ela já está lá!
		   entry é um ponteiro para esta entrada para dentro da árvore, basta ele ser desempilhado. */
   }

	/* Limpa alguns campos da pending_change, para não interferir em commit's futuros */
   sem_gl_info.current_table->pending_changes->pointer     = sem_pt_type_none;
   free(sem_gl_info.current_table->pending_changes->type);
   sem_gl_info.current_table->pending_changes->type        = NULL;
...
/* Realiza várias checagens, dependendo do seletor usado */
int sem_check(sem_check_type check, char* key)
{
   sem_entry* entry = NULL;

	/* Caso a checagem se dê sobre uma entrada específica, busca-se essa entrada
   ...
   if(key)
   {
      ...
      if(!entry) return ERROR;	/* Se a entrada não for encontrada, é impossível
		                               realizar a checagem requisitada sobre ela */
   }

   switch(check)
   {
		/* Verifica se um tipo específico está declarado */
      case sem_check_type_declared:

			/* Caso ele esteja declarado e é um tipo definido pelo usuário, copia o conteúdo deste tipo
			(campos armazenados em uma tabela semântica) para o pending_changes. Este artifício serve para
		...
		/* Checa se a variável especificada foi declarada */
      ...
      case sem_check_var_const_declared:
		...
		/* Checa se a função/procedimento especificado foi delcarado */
      case sem_check_proc_func_declared:
         return   ((entry->category == procedure) || (entry->category == function)) ?
                  SUCCESS : ERROR;

		/* Checa se há uma entrada de qualquer categoria declarada com o identificador especificado */
      ...
      case sem_check_return_allowed:
		...
		/* Checa se os dois tipos do topo da pilha são compatíveis, desempilhando-os */
      case sem_check_attr:
...
/* Função responsável pela impressão dos erros especificados pela professora.
   O uso de prev_line_number foi uma tentativa de fazer os erros serem acusados nas mesmas linhas especificadas pela professora
	 nos casos de teste. Ele ajudou, mas não eliminou todas as discrepâncias. */
...
/* Função responsável pelas mudanças de contexto. */
void sem_context_change(sem_scope_chg_type type)
{
   ...
   switch(type)
   {
      ...
      case sem_scope_global_to_local:
         ...
         sem_gl_info.current_table = sem_gl_info.local_table;	/* Agora o contexto corrente é a tabela local */
         break;

		/* Sai do contexto de função/procedimento para o escopo global */
      case sem_scope_local_to_global:
         sem_table_dispose(sem_gl_info.local_table);
         sem_gl_info.local_table = NULL;

         sem_gl_info.current_table = sem_gl_info.global_table;	/* Agora o contexto corrente é a tabela global */

         break;

		/* Prepara o contexto para a inserção de campos de um registro */
      case sem_scope_register_insert:

			/* Os campos de um registro são armazenados em uma tabela de símbolos */
         ...
         sem_pending_update(sem_upd_more_info, t);	/* Faz o more_info do pending_changes apontar para esta nova tabela alocada 	*/
			...
			/* Deste modo, ao dar o sem_commit posteriormente, o more_info do pending_changes será copiado para o more_info da
			    entrada do registro */

			if(!stack_peek(sem_gl_info.context_stack))     	/* Empty stack */
				last_context = sem_gl_info.current_table;		/* Salva o contexto corrente, caso ele não seja um contexto de registro
      ...
      case sem_scope_register_query:
		...
		/* Encerra consulta/inserção de registro, alterando o contexto de acordo */
      case sem_scope_register_end:
		...
		/* Os dois próximos trechos apenas controlam se o contexto atual permite o comando retorne */
